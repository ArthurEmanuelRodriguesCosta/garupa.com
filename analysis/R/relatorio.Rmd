...
Ao observar o grupo 200-10-100, percebe-se que, mesmo tendo 200 requisições simultâneas, este ainda continua com tempo de rede médio menor que 1 segundo e seu intervalo de confiança confirma isso. O mesmo não ocorre nos demais grupos com a mesma quantidade de requisições. Uma característica comum a estes é a grande quantidade de dados envolvida nas operações. O grupo 200-100-100 possui apenas 100 caronas criadas, porém espera que 100 sejam retornadas. O grupo 200-10-1000 requer apenas 10 caronas, porém existem 1000 caronas criadas. E, por fim, o grupo 200-100-1000 é o que possoui a maior quantidade de recursos, requer 100 respostas e possui 1000 caronas criadas.
...
Separando os grupos por ordem de desempenho, percebe-se que pares são formados. Como já citado anteriormente, os grupos com melhor desempenho são 1-10-100 e 200-10-100, em segundo lugar temos 1-10-1000 e 200-10-1000, seguidos por 1-100-100 e 200-100-100, e por fim 1-100-1000 e 200-100-1000. Os grupos pertencentes ao mesmo par diferem apenas no número de requisições em conjunto, o que reforça a ideia desse fator ter pouca influência no processamento dos dados já que seus valores são tão parecidos.

Tem-se então indícios de que o tempo de processamento está ligado à quantidade de recursos envolvidos na operação, sendo a quantidade de respostas retornadas o principal motivo do aumento de tempo.
...
aov(net_time ~ queries + results + rides, d) %>% summary()
aov(process_time ~ queries + results + rides, d) %>% summary()
aov(bd_time ~ queries + results + rides, d) %>% summary()
```
...
Nessa métrica, os grupos foram divididos em duas partes. Os grupos de ID 1-100-1000, 1-100-100, 200-10-100 e 200-100-100, todos possuindo 100 caronas existentes, apresentam uma média abaixo de 0.01 segundo. Em seguida vemos os demais grupos, que possuem 1000 caronas existentes, com média acima de 0.035 segundo.
